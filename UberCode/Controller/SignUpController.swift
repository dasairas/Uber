...
class SignUpController: UIViewController {
    ...
    private lazy var fullNameContainerView: UIView = {
        let view = UIView().inputContainerView(image: #imageLiteral(resourceName: "ic_mail_outline_white_2x"), textField: fullNameTextField)
    ...
    //MARK: Selectors - Segue de PopView
    ...
    @objc func handleSignUp() {
        ...
        let accountTypeIndex = accountTypeSegmentedControl.selectedSegmentIndex
         ...
         //func del firebase() --> creo base de datos con mail,password y fullname
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            if let error = error {
            print("Failed to register error with \(error)")
                return
            }
            ...
            Database.database().reference().child("users").child(uid).updateChildValues(values, withCompletionBlock:  { (error, ref) in
                print("succesfully register user and saved data...")
            })
        }
    }
    ...
    func configureUI() {
        ...
        view.addSubview(titleLabel) //agrego titlelabel al view
        ...
        view.addSubview(stack) //agrego stack al view
